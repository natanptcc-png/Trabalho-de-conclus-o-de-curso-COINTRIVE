...
function App() {
  ...
  const [authToken, setAuthToken] = useState(() => {
    return localStorage.getItem("userSignedOut") === "true"
      ? null
  ...
  const formatTransactionAmount = (tx) => {
    ...
    const normalizedAmount =
      cleanedAmount.startsWith("+") || cleanedAmount.startsWith("-")
        ? cleanedAmount
        : cleanedAmount;
  ...
  const addItem = async (tx) => {
    ...
    if (body?.wallet !== undefined) {
      setUserProfile((prev) =>
        prev ? { ...prev, wallet: body.wallet } : prev,
      );
      setAuthUser((prev) => (prev ? { ...prev, wallet: body.wallet } : prev));
    }
  ...
  const updateItem = async (id, updates) => {
    ...
    const beforeAmount = existingItem
      ? parseFloat(String(existingItem.amount).replace(/[^\d.-]/g, "")) || 0
      : 0;
    const beforeType = existingItem?.type === "Renda" ? "renda" : "gastos";
    ...
    const nextAmount =
      parseFloat(String(body.amount ?? "").replace(/[^\d.-]/g, "")) || 0;
    const nextType = body.type === "Renda" ? "renda" : "gastos";
    ...
    if (existingItem && body?.wallet !== undefined) {
      setUserProfile((prev) =>
        prev ? { ...prev, wallet: body.wallet } : prev,
      );
      setAuthUser((prev) => (prev ? { ...prev, wallet: body.wallet } : prev));
    ...
    if (existingItem) {
      const previousDelta =
        beforeType === "renda" ? beforeAmount : -beforeAmount;
      const nextDelta = nextType === "renda" ? nextAmount : -nextAmount;
      const walletDelta = nextDelta - previousDelta;

      setUserProfile((prev) =>
        prev
          ? { ...prev, wallet: Number((prev.wallet + walletDelta).toFixed(2)) }
          : prev,
      );
      setAuthUser((prev) =>
        prev
  ...
  const deleteItem = async (id) => {
    ...
    const amount = existingItem
      ? parseFloat(String(existingItem.amount).replace(/[^\d.-]/g, "")) || 0
      : 0;
    const type = existingItem?.type === "Renda" ? "renda" : "gastos";
    ...
    if (existingItem) {
      const walletDelta = type === "renda" ? -amount : amount;
      setUserProfile((prev) =>
        prev
          ? { ...prev, wallet: Number((prev.wallet + walletDelta).toFixed(2)) }
          : prev,
  ...
  useEffect(() => {
    ...
    if (notificationSettings.excessExpenses) {
      ...
      if (monthlyExpenses > 5000 && !generatedNotifications.includes(key)) {
        addNotification({
          type: "excessExpenses",
  ...
  }, [items, notificationSettings, generatedNotifications]);
